...
<!--              </div>-->
              <div>
                <el-radio-group v-model="currentUser.gender">
                  <el-radio :label="0" style="margin-right: 10px">薛定谔的性别</el-radio>
        ...
        this.$http.post(this.$constant.baseURL + "/user/regist", user)
          .then((res) => {
            if (!this.$common.isEmpty(res.data)) {
              ...
              // 在这里添加注册成功的提示
              this.$message({
                message: "注册成功！正在跳转回首页...",
                type: "success",
                duration: 2000 // 消息显示时间，单位毫秒
              });
              // this.$router.push({path: '/'});
              // let userToken = this.$common.encrypt(localStorage.getItem("userToken"));
              // window.open(this.$constant.imBaseURL + "?userToken=" + userToken);  //浏览器新页签打开

               // 设置延迟进行页面跳转，以确保用户能看到注册成功的消息
              setTimeout(() => {
                let userToken = this.$common.encrypt(localStorage.getItem("userToken"));
                this.$router.push({path: '/'}); // 如果要在当前页签跳转
                // 或者如果要重定向到外部链接，则使用下面的代码
                window.location.href = this.$constant.imBaseURL + "?userToken=" + userToken;
              }, 2000); // 此处的延迟时间与消息显示时间相匹配
            }
          })
          .catch((error) => {
            this.$message({
